...
function Skel({ w = '100%', h = 12, radius = 5, className = '' }) {
  return (
    <span
      className={`fb-skel ${className}`}
        ...
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col gap-1.5">
              <Skel w={140} h={14} radius={4} />
              <Skel w={180} h={9} radius={3} />
            </div>
          </div>
          <div className="flex items-center gap-1.5">
            <Skel w={32} h={32} radius={6} />
            <Skel w={80} h={32} radius={6} />
          </div>
        </div>
        {/* Stats strip */}
        <div className="grid grid-cols-4 gap-2">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="border border-slate-200 rounded-lg p-2 sm:p-2.5 bg-slate-50 flex flex-col items-center gap-1.5">
              <Skel w="50%" h={22} radius={4} />
              <Skel w="65%" h={8} radius={3} />
            </div>
          ))}
        </div>
      </div>
...
function TableSkeleton() {
  return (
    <div className="hidden lg:block bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
      {/* Header */}
      <div className="bg-slate-50 border-b border-slate-200 px-4 py-2.5 grid gap-4"
        style={{ gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1fr' }}>
        {['w-16', 'w-12', 'w-20', 'w-12', 'w-20', 'w-14'].map((w, i) => (
          <Skel key={i} w={w} h={9} radius={3} />
        ))}
      </div>
      {/* Rows */}
      {[...Array(4)].map((_, i) => (
        <div key={i} className="border-b border-slate-100 px-4 py-3 grid items-center gap-4"
          style={{ gridTemplateColumns: '2fr 1fr 1fr 1fr 1fr 1fr' }}>
          {/* Account col */}
          <div className="flex items-center gap-2.5">
            <Skel w={36} h={36} radius={10} />
            <div className="flex flex-col gap-1.5 flex-1">
              <Skel w={`${50 + (i % 3) * 20}%`} h={12} radius={4} />
              <Skel w="55%" h={9} radius={3} />
            </div>
          </div>
          {/* Status */}
          <Skel w={60} h={22} radius={6} />
          {/* Ad Accounts */}
          <div className="flex justify-center"><Skel w={32} h={28} radius={8} /></div>
          {/* Token */}
          <div className="flex justify-center"><Skel w={48} h={22} radius={6} /></div>
          {/* Connected */}
          <div className="flex flex-col gap-1">
            <Skel w="70%" h={11} radius={3} />
            <Skel w="45%" h={9} radius={3} />
          </div>
          {/* Actions */}
          <div className="flex justify-end gap-1.5">
            <Skel w={80} h={28} radius={6} />
            <Skel w={80} h={28} radius={6} />
          </div>
        </div>
      ))}
      ...
      <div className="bg-slate-50 border-t border-slate-200 px-4 py-2.5 flex items-center justify-between">
        <Skel w={200} h={10} radius={3} />
        <Skel w={100} h={10} radius={3} />
      </div>
    </div>
  );
}
        ...
        <div key={i} className="bg-white rounded-xl border border-slate-200 shadow-sm p-3">
          ...
          <div className="flex items-start gap-2.5 mb-2.5">
            ...
            <div className="flex-1 flex flex-col gap-1.5">
              <Skel w={`${55 + (i % 3) * 15}%`} h={13} radius={4} />
              <Skel w="40%" h={9} radius={3} />
              <div className="flex gap-1.5 mt-0.5">
                <Skel w={48} h={18} radius={4} />
                <Skel w={52} h={18} radius={4} />
              </div>
            </div>
          </div>
          {/* Stats grid */}
          <div className="grid grid-cols-3 gap-1.5 mb-2.5">
            {[...Array(3)].map((_, j) => (
              <div key={j} className="bg-slate-50 border border-slate-100 rounded-lg p-1.5 flex flex-col items-center gap-1">
                <Skel w="50%" h={16} radius={3} />
                <Skel w="65%" h={8} radius={3} />
              </div>
            ))}
          </div>
  ...
  return (
    <div className="hidden sm:flex items-center justify-between bg-white border border-slate-200 rounded-lg px-3 py-2 mb-3 shadow-sm">
      <div className="flex items-center gap-3">
        {[60, 48, 36, 40, 36, 40].map((w, i) => (
          <Skel key={i} w={w} h={10} radius={3} />
        ))}
      </div>
